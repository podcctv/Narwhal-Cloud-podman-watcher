...
- Incus/Podman 容器首次产生“连接数过高”事件时，Server 会自动下发一次有界深度取证任务，同一轮异常不会重复排队。Agent 在下一次上报中采集入站/出站 IP 数、关联进程数、最多 100 个连接 IP、国家/地区代码、连接方向和进程归属；不采集数据包正文、账号或业务载荷。完整证据与采集时间绑定到告警历史，Telegram 动态告警卡同步显示入站/出站摘要和主要 IP。告警恢复后证据仍按 Server 历史保留周期保存。
...
连接数过高告警的自动深度取证完成后，同一张 Telegram 卡会追加“入站 IP / 进程、出站 IP / 进程、主要 IP 与国家/地区代码”摘要。网页“安全告警历史”中的该告警可展开完整证据，查看最多 100 个 IP 的连接数、方向、归属地和关联进程。
